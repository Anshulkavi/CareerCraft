...
function ResumeUpload() {
  ...
  return (
    <section className="bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 text-center py-12 px-5">
      ...
      {statusVisible && (
        <div className="mt-6 bg-gray-100 dark:bg-gray-800 p-4 rounded-md max-w-2xl mx-auto text-left">
          ...
          {extractedData && (
            <div>
              ...
              <p><strong>💼 Experience:</strong> {extractedData.experience || "N/A"}</p>
              <p><strong>🛠️ Skills:</strong> {extractedData.skills.join(", ")}</p>
            </div>
          )}
        </div>
      )}
      ...
      {jobMatches.length > 0 && (
        <div className="mt-10">
          <h3 className="text-xl font-bold mb-4">Matching Job Opportunities</h3>
          <div className="grid gap-4 max-w-3xl mx-auto">
            {jobMatches.map((job, index) => (
              <div
                key={index}
                className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-md p-4 text-left"
              >
                <h4 className="text-lg font-semibold text-blue-700 dark:text-blue-400 mb-1">{job.title}</h4>
